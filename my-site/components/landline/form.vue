		...
		<div class="row justify-content-between">
			...
			<div class="col-lg-6 mb-6 mb-lg-0">
				<div class="d-flex flex-column h-100 align-items-start">
					<!-- Title -->
					<span class="heading-color d-inline-block bg-light small rounded-3 px-3 py-2">😎 Take back control and express yourself</span>
					<h2 class="my-3">Create your own custom mobile number</h2>
					<p class="mb-0">Getting the mobile number that's just right for you has never been easier. You pay nothing to request a custom mobile number.</p>
					<ol>
						<li>Fill in the form and submit your request.</li>
						<li>Receive all available options by email within three working days.</li>
						<li>Choose the mobile number you like and complete purchase.</li>
					</ol>
					<h6 class="mb-3">Some housekeeping to avoid disappointment</h6>
					<p>✅ Enter a minimum of 6 digits and a maximum of 7 digits that you would like to feature at the end of your number</p>
					<p>⛔ It is not possible to retrieve old or expired mobile numbers</p>
					<p>💷 Prices for custom numbers start from £185</p>
					<p>🚚 Allow 2 to 4 weeks to complete delivery</p>
					<h6 class="my-3">If we can't find it, no-one else can.</h6>
				</div>	
			</div>

			<div class="col-lg-6 col-xl-5 d-flex align-items-center">
				<div id="formContainer" class="card card-body shadow border p-sm-5">
					<!-- SVG decoration -->
					...
					<form  id="requestForm" class="row g-4">
						<div class="col-md-6">
							<label class="form-label heading-color">Your name *</label>
							<input type="text" name="name" id="name" required class="form-control form-control-lg" placeholder="Full name">
						</div>

						<div class="col-md-6">
							<label class="form-label heading-color">Email address *</label>
							<input type="email" name="email" id="email" required class="form-control form-control-lg" placeholder="name@example.com">
							<span id="emailError" class="error"></span>
						</div>

						<div class="col-md-6">
							<label class="form-label heading-color">Contact number</label>
							<input type="text" name="contact" id="contact" class="form-control form-control-lg" placeholder="(xxx) xx xxxx">
						</div>

						<div class="col-md-6">
							<label class="form-label heading-color">Company</label>
							<input type="text" name="company" id="company" class="form-control form-control-lg" placeholder="Company name">
						</div>

						<div class="col-12">
							<label class="form-label heading-color">Request *</label>
							<input type="text" name="request" id="request" required class="form-control form-control-lg" placeholder="Enter last 6 or 7 digits" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" maxlength="7">
							<span id="requestError" class="error"></span>
						</div>

						<!-- Button -->
						<button id="submitRequest" class="btn btn-lg btn-warning mb-0" type="submit">Create my number</button>
					</form>

					<!-- Success message once form submitted -->
					<div id="successCover"
						class="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column justify-content-center align-items-center text-center rounded-3 px-3 bg-dark d-none">
						<i class="bi bi-check-circle-fill text-success" style="font-size: 48px;"></i>
						<p class="mt-3 text-light">You successfully submitted a request - we will get back to you by email within 3 working days.
						</p>
						<button id="submitAnother" class="btn btn-warning">Make another submission</button>
					</div>
				</div>	
			</div>
